...
import { Activity, Bot, Crosshair, MessageSquareText, Radar, ShieldCheck, TerminalSquare, Users } from "lucide-react"
...
export function AppLayout() {
  ...
  return <RedTeamSyncProvider><div className="dashboard-shell min-h-svh bg-[#0a0a0f] text-[#e4e4e7]">
    ...
    <aside className="fixed inset-y-0 left-0 z-20 hidden w-64 flex-col border-r border-white/10 bg-[#111118] lg:flex">
      ...
      <div className="mt-auto p-4"><div className="border border-white/10 bg-[#18181f] p-4"><div className="mb-2 flex items-center gap-2 font-mono text-[10px] uppercase tracking-[0.15em] text-emerald-400"><span className="flex size-1.5 rounded-full bg-emerald-400 status-pulse text-emerald-400" /><Bot className="size-3.5" /> System online</div><p className="font-mono text-[11px] leading-relaxed text-zinc-500">Autonomous agents are monitoring live simulation traffic.</p></div></div>
    </aside>
    <main className="lg:pl-64">
      <header className="sticky top-0 z-10 flex min-h-20 flex-wrap items-center justify-between gap-4 border-b border-white/10 bg-[#0a0a0f]/95 px-5 py-4 backdrop-blur lg:px-10"><div><p className="font-mono text-[10px] uppercase tracking-[0.22em] text-indigo-300">SOC / SIMULATION MODE</p><h1 className="mt-1 text-xl font-semibold tracking-tight text-white">{activePage}</h1></div><div className="flex items-center gap-4 font-mono text-[11px] text-zinc-500"><span className="hidden sm:inline tabular-nums">{clock}</span><span className="flex items-center gap-2.5 text-emerald-400"><span className="flex size-2 rounded-full bg-emerald-400 status-pulse text-emerald-400" />All systems nominal</span></div></header>
